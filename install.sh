...
MY_ARCH="win64"
[[ "$MY_ARCH" = "win32" ]] && MY_ROON_URL="http://download.roonlabs.com/builds/RoonInstaller.exe"
...
check_executable() {
	...
	if ! command -v "${exe}" &>/dev/null; then
		...
		echo "Please install ${exe} using your distribution package manager."
		echo
		exit 1
	fi
}
